...
export default function LiveActivityPanel({ onSelectStop, currentStopId }) {
  ...
  const fetchLiveActivity = useCallback(async (isBackground = false) => {
    ...
    try {
      // Fetch limit=0 to load all active stops with live predictions for the map & risk distribution
  ...
  return (
    <>
      <div className="live-activity-grid">
        {/* Live Activity Radar Column (Top 20 Stops Only) */}
        <div className="glass-panel" style={{ padding: '1.25rem' }}>
          <div className="section-header" style={{ marginBottom: '1rem' }}>
            <div className="section-title">
              <Activity size={20} style={{ color: 'var(--accent-cyan)' }} />
              <span>Live Activity Radar</span>
              <span className="badge-count" title="Top 20 active high-risk stops across Stockholm network">
                Top 20 Active Stops
              </span>
            </div>
        ...
        {/* Risk Distribution Column (Uses All Predictions) */}
        <RiskDistributionCard stops={stops} isLoading={isLoading} />
      </div>

      {/* Live Risk Map Component (Plots All Active Stops) */}
      <LiveRiskMap
        stops={stops}
