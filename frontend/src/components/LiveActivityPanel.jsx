...
export default function LiveActivityPanel({ onSelectStop, currentStopId }) {
  ...
  const fetchLiveActivity = useCallback(async (isBackground = false) => {
    ...
    try {
      // Fetch limit=100 so all active stops and trips are loaded for radar & distribution calculation
      const data = await getLiveActivity(100);
      setStops(data.stops || []);
      setError(null);
    } catch (err) {
